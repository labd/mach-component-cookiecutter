...
let defaultOptions = {
  ...
  async fillInfo(ctx: Context) {
    ctx.__logInfo = ctx.state.__logInfo = {}
  },

  async fillError(ctx: Context) {
  ...
  onStartFormat(ctx: Context, _info: any) {
  ...
  onErrorFormat(ctx: Context, _info: any) {
    return `[ERROR] ${ctx.method} ${ctx.url}`
  },

  async onError(ctx: Context, err: Error) {
    // @ts-ignore
  ...
  onEndFormat(ctx: Context, timeTaken: number, _info: any) {
